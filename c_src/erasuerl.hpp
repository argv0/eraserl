...
template <typename T> bool erasuerl_is_erasure(const T &block);
template <typename T> char *erasuerl_block_address(const T &block);
template <typename T> size_t erasuerl_block_size(const T &block);
template <typename T> T erasuerl_new_block(size_t size);
template <typename T> void erasuerl_realloc_block(T &block, size_t size);
template <typename T> void erasuerl_free_block(T &block) {}
...
make_decode_state(erasuerl_handle *h, std::vector<T> &user_blocks,
